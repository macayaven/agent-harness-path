...
`lessons/S04-structured-generation.html`,
`notebooks/s04_structured_generation_toy.ipynb`, `labs/s04_schema.md`,
`trivia_host/tools.py` (`propose_round_spec`), `labs/schemas.py`,
`labs/spec_schema.py` (`validate_spec`).

## The gap

The toy validates a small JSON blob and retries. The host’s
`propose_round_spec` already:
