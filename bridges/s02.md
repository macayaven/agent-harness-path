...
Tutor a learner on **S02**. The host already has `run_naive` and `run_engine`.
The work is to **read the report**, not to invent a second eval framework.

## Files

| Role | Path |
| --- | --- |
| Theory | `lessons/S02-golden-evals.html` |
| Toy | `notebooks/s02_scripted_user_eval_toy.ipynb` |
| Protocol | `labs/s02_evals.md` |
...
S01 loop/tools/client constraints still apply (`bridges/s01.md` wire table).

## The gap

The toy shows a miniature table: same scripted user, two impls, explain every
cell. The lab runner writes `naïve n/6 vs engine n/6` against **committed
cassettes**. Several engine checks encode governors built in **later** sessions
(spec tool, PII, ceiling). On a from-scratch host those fails are a valid
baseline. On this companion cut the engine column should already pass; naïve
**should still fail** some cells. That fail is the product argument.
