...
Core: `lessons/S05-consent-gate.html`,
`notebooks/s05_consent_gate_toy.py`, `cafe/consent.py` (`run_shift`,
`check_fire`, `fire_requested`) — the gate before `fire_ticket`. Optional trivia
lab: `labs/s05_consent.md`, `run_engine` consent block in
...
uv run marimo edit notebooks/s05_consent_gate_toy.py
