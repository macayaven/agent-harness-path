...
`lessons/S06-layered-detection.html`,
`notebooks/s06_layered_detection_toy.ipynb`, `labs/s06_policy.md`,
`policy_hit` / `medical_advice_hit` / repair prelude in
`trivia_host/engine.py`, PII regex in `spec_schema.py`, deck answers in
`labs/deck.py`.

**Hard rule:** pub-quiz policy only (spoilers, off-limits, PII in clues).
No crisis protocol, no clinical handoff, no emergency numbers.

## The gap

The toy is a keyword floor plus a second pass on a tiny domain. The host
already:
