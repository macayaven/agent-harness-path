...
You type in `trivia_host/` — this companion cut **ships a complete host**
there so `--replay` runs without filling stubs. `reference/` is the same
spine for CI `--impl reference`.
...
- `trivia_host/` — complete host in this companion cut (study with bridges)
