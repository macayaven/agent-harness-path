...
Stdlib only. Run after `tools/build.py`. Skips mailto, data, and in-page
...
Checks <a href>, <script src>, <img src>, <link href>, <video src>, <source src>.
  ...
  uv run python tools/check_links.py         # generated HTML in sessions/
  uv run python tools/check_links.py --http  # also GET unique http hrefs
"""
...
def display(page: Path, root: Path) -> str:
    return str(page.relative_to(root))


def check_relative(
    pages: list[Path], root: Path
) -> tuple[list[str], int]:
    missing: list[str] = []
    checked = 0
    for page in pages:
        ...
        for ref in parser.refs:
            ...
            if not resolved.exists():
                missing.append(f"{display(page, root)} -> {ref}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=HERE.parent / "sessions",
        help="Directory of HTML to check (default: sessions/)",
    )
    parser.add_argument(
        "--http",
        action="store_true",
    ...
    if not pages:
        ...
        return 1
    missing, checked = check_relative(pages, root)
    if missing:
