...
# Notebook-only diagrams: sources are <session>/public/diagrams/<asset>.mmd,
# next to their renders. The session toy embeds them as markdown figures
# (`![...](public/diagrams/<asset>.svg)`): marimo serves notebook-adjacent
# files only from a public/ directory, so this one reference renders both
# statically (resolved against the notebook file) and at runtime (served by
# the kernel). No mermaid runs at view time.
NOTEBOOK_SESSIONS = {
    's03-window': 's03-context-engineering',
    's05-consent': 's05-consent-gate',
    's08-replay': 's08-observability-replay',
    's11-budget': 's11-budgets-routing',
    's12-judge': 's12-judge-calibration',
}


def session_dir(slug):
    """Directory holding a slug's page, relative to the repo root anchor."""
    if slug in ('index', 'study-plan'):
        return SESSIONS
    return SESSIONS / ('s' + slug[1:])


def lesson_source(slug):
    """Authoring Markdown for a slug: sessions/<dir>/lesson.md."""
    directory = session_dir(slug)
    if directory == SESSIONS:
        return directory / f'{slug}.md'
    return directory / 'lesson.md'


def diagram_path(slug, asset):
    """Committed SVG for a lesson asset inside its session's public/ dir."""
    return session_dir(slug) / 'public' / 'diagrams' / f'{asset}.svg'
...
def static_image(slug, index, source):
    ...
    svg = path.read_bytes()
    entry = json.loads(RECEIPT.read_text())['diagrams'].get(asset, {})
    if entry.get('inputs') != inputs(source) or entry.get('svg_sha256') != sha(svg):
        raise ValueError(f'stale diagram for {asset}; run uv run --group diagrams python tools/render_diagrams.py')
    ...
    return (
        ...
        f'<img class="static-diagram" src="public/diagrams/{asset}.svg" '
