...
**Hands-on:** [`notebooks/s12_judge_calibration_toy.py`](../notebooks/s12_judge_calibration_toy.py) — runs against **your** model.
**Video:** [Gemini Notebook overview](videos/S12-judge-calibration.mp4) — generated with Google Gemini Notebook (formerly NotebookLM); recorded against an earlier cut of this path, so it still uses the previous toy domain. Preview or review, never a substitute for the notebook.
...
Open [`notebooks/s12_judge_calibration_toy.py`](../notebooks/s12_judge_calibration_toy.py).
...
uv run marimo edit notebooks/s12_judge_calibration_toy.py
```
...
**[S13 — Rebuild from memory](S13-rebuild-from-memory.html)** is the closed-book audit.
