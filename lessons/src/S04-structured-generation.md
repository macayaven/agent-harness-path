...
| Anthropic shipped native Structured Outputs — public beta Nov 2025 (Sonnet 4.5, Opus 4.1), GA in 2026: grammar-constrained `output_config.format` plus strict tool use ([docs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs)) | **newer than this session** | Constrained decoding is now table stakes on every major API. The portable validate-retry fallback still earns its place on routes that lag — and for the semantic layer no sampler can fix. |
...
| Schemas became the agent-to-agent contract: MCP tools declare `inputSchema`/`outputSchema` in JSON Schema ([spec](https://modelcontextprotocol.io/specification/2025-06-18/server/tools)) | **recognize** | Your validator skills transfer verbatim. A tool schema is the same contract, one hop further downstream. |
