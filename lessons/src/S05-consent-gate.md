...
| MCP elicitation: a server may pause tool execution to request structured user input mid-run ([spec 2025-06-18](https://modelcontextprotocol.io/specification/2025-06-18/client/elicitation)) | **recognize** | Pause-and-ask, standardized. Legitimate for missing parameters; as a re-consent channel it formalizes the ambush question. |
...
| Human oversight is now law for high-risk AI in the EU: systems must be built so assigned humans can effectively oversee and intervene ([AI Act, Article 14](https://artificialintelligenceact.eu/article/14/)) | **recognize** | For high-risk deployments the gate stops being taste and becomes a compliance artifact, with an evidence trail. Worth knowing before someone asks for yours. |
